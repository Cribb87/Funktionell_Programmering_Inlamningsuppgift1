...
public class RewriteMe {
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(c -> c.getCategory().equals(category)).map(c -> c.question).collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().anyMatch(a -> a.getAllAnswers().toString().contains(answerCandidate)); //.filter(a -> a.getAllAnswers().stream().anyMatch(a -> a.equals(answerCandidate)));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return questions.stream().map(a -> a.getAllAnswers().contains(answerCandidate)).filter(b -> b).collect(Collectors.toList()).size();
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c){
        return questions.stream().filter(a -> a.getCategory().equals(c)).flatMap(q -> q.getAllAnswers().stream()).reduce("",(x, y) -> x.length() < y.length() ? y:x);
    }
